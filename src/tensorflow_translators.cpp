...
namespace {
template <typename T>
T extract_scalar_const_value(const std::shared_ptr<Node> &node,
                             const std::string &const_name) {
  auto const_node = as_type_ptr<Constant>(node);
  FRONT_END_GENERAL_CHECK(const_node, "Conversion expects " + const_name +
                                          " to be constant.");
  std::vector<T> const_value = const_node->cast_vector<T>();
  FRONT_END_GENERAL_CHECK(const_value.size() == 1, "Conversion expects " +
                                                       const_name +
                                                       " to be a scalar.");
...
OutputVector translate_sentencepiece_op(const NodeContext &node) {
  ...
  auto sp_model_const = std::make_shared<Constant>(
      element::u8, Shape{str_spm_model.size()}, str_spm_model.data());
  return {sp_model_const};
}

NamedOutputVector translate_ragged_tensor_to_sparse(const NodeContext &node) {
  // this is custom translator that converts a sub-graph with SentencePieceOp,
  // SentencePieceTokenizer, and RaggedTensorToSparse operation- into a custom
  // operation SentencepieceTokenizerExtensionOp
  FRONT_END_GENERAL_CHECK(node.get_input_size() > 0,
                          "RaggedTensorToSparse expects at least one input.");
  ...
  if (ov::as_type_ptr<ov::op::util::FrameworkNode>(
          node.get_input(0).get_node_shared_ptr())) {
    auto sp_tokenize_op = node.get_input(0).get_node_shared_ptr();

    FRONT_END_GENERAL_CHECK(
        sp_tokenize_op->get_input_size() > 6,
    ...
    auto sp_model_const = as_type_ptr<Constant>(
        sp_tokenize_op->input_value(0).get_node_shared_ptr());
    FRONT_END_GENERAL_CHECK(
        sp_model_const,
        "Conversion expects SentencePiece model to be constant.");

    // prepare input
    auto inputs = sp_tokenize_op->input_value(1);

    // extract values for nbest_size, alpha, add_bos, add_eos, reverse
    // attributes
    auto nbest_size = extract_scalar_const_value<int32_t>(
        sp_tokenize_op->input_value(2).get_node_shared_ptr(), "nbest_size");
    auto alpha = extract_scalar_const_value<float>(
        sp_tokenize_op->input_value(3).get_node_shared_ptr(), "alpha");
    auto add_bos = extract_scalar_const_value<bool>(
        sp_tokenize_op->input_value(4).get_node_shared_ptr(), "add_bos");
    auto add_eos = extract_scalar_const_value<bool>(
        sp_tokenize_op->input_value(5).get_node_shared_ptr(), "add_eos");
    auto reverse = extract_scalar_const_value<bool>(
        sp_tokenize_op->input_value(6).get_node_shared_ptr(), "reverse");

    OutputVector inputs_vector = OutputVector{sp_model_const, inputs};

    // create a node with custom operation
    auto sp_tokenizer_ext = std::make_shared<SentencepieceTokenizer>(
        inputs_vector, nbest_size, alpha, add_bos, add_eos, reverse);
    FRONT_END_GENERAL_CHECK(sp_tokenizer_ext->get_output_size() == 3,
                            "Internal error: SentencepieceTokenizer operation "
                            "extension must have three outputs.");
    ...
    FRONT_END_GENERAL_CHECK(
        ...
        "RaggedTensorToSparse is supported only for one dimension raggedness");
    auto rt_nested_splits = node.get_input(0);
    auto rt_dense_values = node.get_input(1);

    rt_nested_splits =
        std::make_shared<Convert>(rt_nested_splits, ov::element::i32);

    // compute vectors of begins and ends
    auto rpt_shape =
        std::make_shared<ShapeOf>(rt_nested_splits, ov::element::i32)
            ->output(0);
    auto const_one = std::make_shared<Constant>(ov::element::i32, Shape{}, 1);
    auto rpt_shape_minus_one =
        std::make_shared<Subtract>(rpt_shape, const_one)->output(0);
    auto begins_start =
        std::make_shared<Constant>(ov::element::i32, Shape{1}, 0);
    ...
    auto begins = std::make_shared<Slice>(rt_nested_splits, begins_start,
                                          rpt_shape_minus_one, step);
    auto ends =
        std::make_shared<Slice>(rt_nested_splits, ends_start, rpt_shape, step);
    ...
    auto reduce_axis =
        std::make_shared<Constant>(ov::element::i32, Shape{1}, 0);
    longest_row_size =
        std::make_shared<ReduceMax>(longest_row_size, reduce_axis, true);

    sparse_dense_shape = std::make_shared<Concat>(
        ov::OutputVector{longest_batch, longest_row_size}, 0);
    sparse_indices =
        std::make_shared<RaggedToSparse>(ov::OutputVector{begins, ends})
            ->output(0);
    sparse_values = rt_dense_values;

    sparse_indices =
        std::make_shared<Convert>(sparse_indices, ov::element::i64);
    sparse_dense_shape =
        std::make_shared<Convert>(sparse_dense_shape, ov::element::i64);
  }
...
ov::OutputVector
translate_case_fold_utf8(const ov::frontend::NodeContext &node) {
  FRONT_END_GENERAL_CHECK(node.get_input_size() == 1,
                          "CaseFold expects only 1 input");
  return {post_translate_string_tensor_output(
      std::make_shared<CaseFold>(
          pre_translate_string_tensor_input(node.get_input(0)))
          ->outputs())};
}

ov::OutputVector
translate_normalize_utf8(const ov::frontend::NodeContext &node) {
  FRONT_END_GENERAL_CHECK(node.get_input_size() == 1,
                          "NormalizeUTF8 expects only 1 input");
  return {post_translate_string_tensor_output(
      std::make_shared<NormalizeUnicode>(
          pre_translate_string_tensor_input(node.get_input(0)),
...
ov::OutputVector
translate_static_regex_replace(const ov::frontend::NodeContext &node) {
  auto node_name = node.get_name();
  FRONT_END_GENERAL_CHECK(node.get_input_size() == 1,
                          "StaticRegexReplace expects only 1 input");
  ...
  auto string_pack_result = post_translate_string_tensor_output(
      std::make_shared<RegexNormalization>(inputs, replace_global)->outputs());
  set_node_name(node_name, string_pack_result.get_node_shared_ptr());
  return {string_pack_result};
}

ov::OutputVector
translate_regex_split_with_offsets(const ov::frontend::NodeContext &node) {
  FRONT_END_GENERAL_CHECK(node.get_input_size() == 3,
                          "RegexSplitWithOffsets expects 3 inputs");
  ov::OutputVector inputs =
      pre_translate_string_tensor_input(node.get_input(0));
  auto delim_regex_pattern = node.get_input(1).get_node()->input_value(
      2); // use u8 part of packed string tensor as we are expecting a scalar
          // string: TODO: verify it is really there
  inputs.push_back(delim_regex_pattern);
  // TODO: Use node.get_input(2) with keep_delim_regex_pattern, most likely it
  // should be handled in another RegexSplit with `isolate` behaviour
  ...
  return {post_translate_ragged_tensor_output(
      {outputs[0], outputs[1], flatten_string_tensor})};
}

ov::OutputVector translate_wordpiece_tokenize_with_offsets(
    const ov::frontend::NodeContext &node) {
  FRONT_END_GENERAL_CHECK(node.get_input_size() == 2,
                          "WordpieceTokenizeWithOffsets expects 2 inputs");
  ov::OutputVector inputs =
      pre_translate_ragged_string_tensor_input(node.get_input(0));

#if USE_STRING_TENSORS
  // It may seem enough to call pre_translate_string_tensor_input that will
  // override Parameter element type in case if string tensors are not used. But
  // a Parameter is still required to be overridden even if string tensors are
  // used because in TF model it is represented not as a string tensor, but as a
  // resource with hash table for lookup that we cannot interpret and have to
  // replace by 1D string tensor.
  override_parameter(node.get_input(1).get_node_shared_ptr(), element::string,
  ...
  // FIXME: Cannot set real value for unk_token_id from attributes because it is
  // not known in this operation
  // TODO: Set other attributes.
  auto wp_tokenizer = std::make_shared<WordpieceTokenizer>(
      inputs, node.get_attribute<std::string>("suffix_indicator"),
...
ov::OutputVector translate_string_lower(const ov::frontend::NodeContext &node) {
  auto node_name = node.get_name();
  FRONT_END_GENERAL_CHECK(node.get_input_size() == 1,
                          "StringLower expects only 1 input");
  auto encoding = node.get_attribute<std::string>("encoding", "");
  ov::OutputVector inputs =
      pre_translate_string_tensor_input(node.get_input(0));
  auto string_lower_result = post_translate_string_tensor_output(
      std::make_shared<CaseFold>(inputs, encoding)->outputs());
  set_node_name(node_name, string_lower_result.get_node_shared_ptr());
  return {string_lower_result};
}

OutputVector translate_lookup_table_find_op(
    const ov::frontend::tensorflow::NodeContext &node) {
  FRONT_END_GENERAL_CHECK(
      node.get_input_size() == 3,
      "LookupTableFind or LookupTableFindV2 expects 3 inputs");
  auto table_handle = as_type_ptr<ov::frontend::tensorflow::HashTable>(
      node.get_input_by_reference(0).get_node_shared_ptr());
  TENSORFLOW_OP_VALIDATION(
      node, table_handle,
      "[TensorFlow Frontend] internal error: LookupTableFind operation expects "
      "table_handle by the first input");
  ...
  TENSORFLOW_OP_VALIDATION(
      ...
      key_type.is_integral_number() ||
          (key_type == element::string && value_type == element::i64),
      "[TensorFlow Frontend] internal error: LookupTableFind is supported two "
      "cases: "
  ...
  auto target_shape = std::make_shared<Constant>(element::i32, Shape{1},
  ...
  TENSORFLOW_OP_VALIDATION(
      node, key_type != element::string,
      "[TensorFlow Frontend] internal error: LookupTableFind operation with "
      "string key is only supported for integral values");

  // update all values with default value and all keys
  auto default_value_shape = std::make_shared<Constant>(
      element::i32, Shape{1}, std::vector<int32_t>{1});
  default_value =
      std::make_shared<Reshape>(default_value, default_value_shape, false);
  all_values =
      std::make_shared<Concat>(OutputVector{all_values, default_value}, 0);
  auto num_keys = std::make_shared<ShapeOf>(all_keys, element::i64)->output(0);
  auto scalar_shape = std::make_shared<Constant>(element::i32, Shape{0},
                                                 std::vector<int32_t>{});
  num_keys = std::make_shared<Reshape>(num_keys, scalar_shape, false);
  num_keys = std::make_shared<Convert>(num_keys, key_type);

  // compute mask which keys are not valid and for which default value must be
  // used
  auto unsqueeze_axis = std::make_shared<Constant>(element::i32, Shape{1},
                                                   std::vector<int32_t>{-1});
  auto unsqueeze_keys = std::make_shared<Unsqueeze>(keys, unsqueeze_axis);
  auto equal_mask =
      std::make_shared<Equal>(all_keys, unsqueeze_keys)->output(0);
  auto reduce_equal_mask =
      std::make_shared<ReduceLogicalOr>(equal_mask, unsqueeze_axis, false);

  // map keys to new keys from range [0, n], n index will be for out-of-range
  // keys
  ...
  // 2. generate a range [0, n-1] that will be multiplied to mask for
  // computation of new keys
  auto new_all_keys =
      std::make_shared<Range>(const_zero, num_keys, const_one, key_type);
  // 3. compute new keys
  auto reduce_axis = std::make_shared<Constant>(element::i32, Shape{1},
                                                std::vector<int32_t>{-1});
  ...
  auto gather_axis = std::make_shared<Constant>(element::i32, Shape{1},
                                                std::vector<int32_t>{0});
  auto lookup_values =
      std::make_shared<Gather>(all_values, new_keys, gather_axis);
...
NamedOutputVector
translate_string_split(const ov::frontend::NodeContext &node) {
  auto node_name = node.get_name();
  FRONT_END_GENERAL_CHECK(
      node.get_input_size() == 2,
      "StringSplitV2 expects two inputs (1D input and separator)");
  auto input = node.get_input(0);
  ov::OutputVector unpacked_input = pre_translate_string_tensor_input(input);
  auto sep_const =
      ov::as_type_ptr<Constant>(node.get_input(1).get_node_shared_ptr());
  TENSORFLOW_OP_VALIDATION(node, sep_const,
                           "[TensorFlow Frontend] internal error: only "
                           "constant separator is supported for StringSplitV2");
  auto sep_value = sep_const->cast_vector<std::string>();
  TENSORFLOW_OP_VALIDATION(
      node, sep_value.size() == 1,
      "[TensorFlow Frontend] inconsistent model: separator must be a scalar");
  auto sep =
      std::make_shared<Constant>(element::u8, Shape{sep_value[0].length()},
                                 (const void *)sep_value[0].data())
          ->output(0);
  if (sep_value[0] == "") {
    // default case that means string elements will be removed from leading and
    ...
    unpacked_input =
        std::make_shared<RegexNormalization>(inputs, true)->outputs();
    std::string new_sep_value = "[\\s\\p{Zs}]+";
    sep = std::make_shared<Constant>(element::u8, Shape{new_sep_value.length()},
                                     (const void *)new_sep_value.data());
  }
  ...
  auto squeeze_axis = std::make_shared<Constant>(element::i32, Shape{1},
                                                 std::vector<int32_t>{0});
  ...
  auto regex_split_outputs =
      std::make_shared<RegexSplit>(
          ov::OutputVector{ragged_begins, ragged_ends, unpacked_input[0],
                           unpacked_input[1], unpacked_input[2], sep},
          nullptr, "remove", false, maxsplit)
          ->outputs();

  // compute sparse tensor indices
  auto indices =
      std::make_shared<RaggedToSparse>(
          ov::OutputVector{regex_split_outputs[0], regex_split_outputs[1]})
          ->output(0);
  ...
  auto values = post_translate_string_tensor_output(ov::OutputVector{
      regex_split_outputs[2], regex_split_outputs[3], regex_split_outputs[4]});
  values.set_names({node_name + ":1"});

  // compute a shape of output tensor in a dense form
  // compute maximum number of string elements per batch in output tensor after
  // split
  auto max_num_per_batch =
      std::make_shared<Subtract>(regex_split_outputs[1], regex_split_outputs[0])
          ->output(0);
  auto reduction_axes = std::make_shared<Constant>(element::i32, Shape{1},
                                                   std::vector<int32_t>{0});
  max_num_per_batch =
      std::make_shared<ReduceMax>(max_num_per_batch, reduction_axes, true);
  auto shape = std::make_shared<Concat>(
                   ov::OutputVector{input_shape, max_num_per_batch}, 0)
                   ->output(0);
...
ov::OutputVector
translate_ragged_tensor_to_tensor(const ov::frontend::NodeContext &node) {
  auto node_name = node.get_name();
  auto node_input_size = node.get_input_size();
  TENSORFLOW_OP_VALIDATION(
      node, node_input_size == 4 || node_input_size == 5,
      "[TensorFlow Frontend] internal error: RaggedTensorToTensor is supported "
      "only with one row partition tensor");
  auto shape = node.get_input(0);
  auto values = node.get_input(1);
  auto default_value = node.get_input(2);
  auto row_partition_types =
      node.get_attribute<std::vector<std::string>>("row_partition_types");
  TENSORFLOW_OP_VALIDATION(
      node,
      (row_partition_types == std::vector<std::string>{"ROW_SPLITS"}) ||
          (row_partition_types ==
           std::vector<std::string>{"FIRST_DIM_SIZE", "VALUE_ROWIDS"}),
      "[TensorFlow Frontend] internal error: RaggedTensorToTensor is supported "
      "only for ROW_SPLITS type");
  // currently we support only shape for 2D tensor in output
  // for example, shape can be equal to [2, 5] or [-1, 8]
  TENSORFLOW_OP_VALIDATION(
      node,
      shape.get_partial_shape().is_static() &&
          shape.get_shape() == ov::Shape{2},
      "[TensorFlow Frontend] internal error: RaggedTensorToTensor is supported "
      "only for 2D ragged tensor on input");
  ...
  if (row_partition_types == std::vector<std::string>{"ROW_SPLITS"}) {
    ...
    row_partition_tensor =
        std::make_shared<Convert>(row_partition_tensor, ov::element::i32);

    // compute vectors of begins and ends
    auto rpt_shape =
        std::make_shared<ShapeOf>(row_partition_tensor, ov::element::i32)
            ->output(0);
    auto const_one = std::make_shared<Constant>(ov::element::i32, Shape{}, 1);
    auto rpt_shape_minus_one =
        std::make_shared<Subtract>(rpt_shape, const_one)->output(0);
    ...
    auto ends_start = std::make_shared<Constant>(ov::element::i32, Shape{1}, 1);
    auto step = std::make_shared<Constant>(ov::element::i32, Shape{1}, 1);
    begins = std::make_shared<Slice>(row_partition_tensor, begins_start,
                                     rpt_shape_minus_one, step);
    ends = std::make_shared<Slice>(row_partition_tensor, ends_start, rpt_shape,
                                   step);
    longest_batch = rpt_shape_minus_one;

    // since shape can contain -1 dimension that means dimension size will be
    // defined automatically such shape must be adjusted based on other inputs
    // to RaggedTensorToTensor compute the longest row in a tensor
    ...
    first_dim_size =
        std::make_shared<Convert>(first_dim_size, ov::element::i32);
    auto new_first_dim_size_shape =
        std::make_shared<Constant>(ov::element::i32, Shape{1}, 1);
    first_dim_size = std::make_shared<Reshape>(first_dim_size,
                                               new_first_dim_size_shape, false);
    value_rowids = std::make_shared<Convert>(value_rowids, ov::element::i32);

    auto ragged_to_ragged = std::make_shared<RaggedToRagged>(
        ov::OutputVector{value_rowids, first_dim_size});
    begins = ragged_to_ragged->output(0);
    ends = ragged_to_ragged->output(1);
    longest_batch = first_dim_size;

    // compute longest_row_size
    auto scalar_shape = std::make_shared<Constant>(ov::element::i32, Shape{0},
                                                   std::vector<int32_t>{});
    first_dim_size =
        std::make_shared<Reshape>(first_dim_size, scalar_shape, false);
    auto const_zero = std::make_shared<Constant>(ov::element::i32, Shape{}, 0);
    auto const_one = std::make_shared<Constant>(ov::element::i32, Shape{}, 1);
    auto range_row_ids = std::make_shared<Range>(const_zero, first_dim_size,
                                                 const_one, ov::element::i32)
                             ->output(0);
    auto unsqueeze_axis =
        std::make_shared<Constant>(ov::element::i32, Shape{1}, 1)->output(0);
    range_row_ids = std::make_shared<Unsqueeze>(range_row_ids, unsqueeze_axis);
    unsqueeze_axis =
        std::make_shared<Constant>(ov::element::i32, Shape{1}, 0)->output(0);
  ...
  auto ragged_to_dense = std::make_shared<RaggedToDense>(
                             ov::OutputVector{begins, ends, values,
                                              longest_row_size, default_value})
                             ->output(0);

  // adjust shape value since it can contain -1 value that means a dimension
  // must be deduced based on minimal dimension size to store output tensor
  auto replace_shape = std::make_shared<Concat>(
                           ov::OutputVector{longest_batch, longest_row_size}, 0)
                           ->output(0);
  auto const_zero = std::make_shared<Constant>(ov::element::i32, Shape{}, 0);
  ...
  auto pads_begin = std::make_shared<Constant>(ov::element::i32, Shape{2},
  ...
  auto result_dense_tensor =
      std::make_shared<Pad>(ragged_to_dense, pads_begin, pads_end, pad_value,
...
ov::OutputVector translate_equal(const ov::frontend::NodeContext &node) {
  ...
  auto is_complex1 =
      ov::as_type_ptr<ComplexTypeMark>(input1.get_node_shared_ptr());
  auto is_complex2 =
      ov::as_type_ptr<ComplexTypeMark>(input2.get_node_shared_ptr());
  if (is_complex1 && is_complex2) {
    // Handle the case where both inputs are complex.
    auto complex_tensor1 = is_complex1->input_value(0);
    auto complex_tensor2 = is_complex2->input_value(0);

    // Perform the equality operation on the extracted complex tensors.
    auto equal_op =
        std::make_shared<ov::op::v1::Equal>(complex_tensor1, complex_tensor2);

    // Reduce the equality results to determine overall equality.
    // Reduce along the last dimension to get a scalar result.
    auto reduce_axes = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{1}, std::vector<int32_t>{-1});
    auto equal_reduced = std::make_shared<ov::op::v1::ReduceLogicalAnd>(
        equal_op, reduce_axes, false);

    // Set the output to the reduced result.
    result = equal_reduced;
  }

  if (input1.get_element_type() == ov::element::string ||
      input2.get_element_type() == ov::element::string) {
    ov::OutputVector unpacked_input1 =
        pre_translate_string_tensor_input(input1);
    ov::OutputVector unpacked_input2 =
        pre_translate_string_tensor_input(input2);
...
ov::OutputVector
translate_string_to_hash_bucket_fast(const ov::frontend::NodeContext &node) {
  auto node_name = node.get_name();
  auto node_input_size = node.get_input_size();
  TENSORFLOW_OP_VALIDATION(node, node_input_size == 1,
                           "[TensorFlow Frontend] inconsistent model: "
                           "StringToHashBucketFast must have one input");
  auto input = node.get_input(0);
  auto num_buckets = node.get_attribute<int64_t>("num_buckets");
  TENSORFLOW_OP_VALIDATION(
      node, num_buckets > 0,
      "[TensorFlow Frontend] inconsistent model: num_buckets for "
      "StringToHashBucketFast must be positive");

  ov::OutputVector unpacked_input = pre_translate_string_tensor_input(input);
  ov::Output<ov::Node> result =
      std::make_shared<StringToHashBucket>(unpacked_input, num_buckets);
